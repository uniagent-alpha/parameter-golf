import argparse, base64, collections, copy, fcntl, glob, io, lzma, math, os, sys
from pathlib import Path
import random, re, subprocess, time, uuid, numpy as np, sentencepiece as spm, torch, torch.distributed as dist, torch.nn.functional as F
...
os.environ.setdefault("TORCHINDUCTOR_CACHE_DIR", "/workspace/.torch_inductor")

# ===== Fused softcapped CE (Triton) =====
...
def _softcapped_ce_fwd_kernel(
    ...
    for off in range(0, n_cols, block_size):
        ...
        mask = cols < n_cols
        val = tl.load(
            logits_row_ptr + cols * stride_logits_v, mask=mask, other=-float("inf")
        ).to(tl.float32)
...
def _softcapped_ce_bwd_kernel(
    ...
    for off in range(0, n_cols, block_size):
        cols = off + tl.arange(0, block_size)
        mask = cols < n_cols
        val = tl.load(logits_row_ptr + cols * stride_logits_v, mask=mask, other=0.0).to(
            tl.float32
        )
...
def softcapped_cross_entropy(logits, targets, softcap, reduction="mean"):
    losses, _ = torch.ops.pgsubmission1draft7fusedce.softcapped_ce(
...
class Hyperparameters:
    data_path_root = os.environ.get("DATA_PATH", "/data/datasets/")
    tokenizer_path_root = os.environ.get("TOKENIZER_PATH", "/data/tokenizers/")
    ...
    sparse_attn_gate_enabled = bool(
        int(os.environ.get("SPARSE_ATTN_GATE_ENABLED", "1"))
    )
    ...
    if caseops_enabled:
        datasets_dir = os.path.join(
            data_path_root, "fineweb10B_sp8192_lossless_caps_caseops_v1_reserved"
        )
        ...
        datasets_dir = os.path.join(data_path_root, f"fineweb10B_sp{vocab_size}")
        tokenizer_path = os.path.join(
            tokenizer_path_root, f"fineweb_{vocab_size}_bpe.model"
        )
    train_files = os.path.join(datasets_dir, "fineweb_train_*.bin")
    val_files = os.path.join(datasets_dir, "fineweb_val_*.bin")
    val_bytes_files = os.path.join(datasets_dir, "fineweb_val_bytes_*.bin")
    artifact_dir = os.environ.get("ARTIFACT_DIR", "")
    logfile = os.path.join(artifact_dir, f"{run_id}.txt") if artifact_dir else None
    quantized_model_path = (
        os.path.join(artifact_dir, "final_model.ptz")
        if artifact_dir
        else "final_model.ptz"
    )
...
class ValidationData:
    def __init__(self, h, device):
        ...
        if int(self.sp.vocab_size()) != h.vocab_size:
            raise ValueError(
                f"VOCAB_SIZE={h.vocab_size} != {int(self.sp.vocab_size())}"
            )
...
def load_validation_tokens(pattern, seq_len):
    ...
    if usable <= 0:
        raise ValueError("val too short")
...
def load_validation_byte_sidecar(pattern, seq_len, expected_len):
    ...
    if bytes_full.numel() < expected_len:
        raise ValueError("sidecar too short")
...
class DocumentPackingLoader:
    ...
    def __init__(self, h, device, cu_bucket_size=64):
        ...
        if not all_files:
            raise FileNotFoundError(h.train_files)
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        self.neftune_alpha = getattr(h, "neftune_alpha", 0.0)
        self._in_ttt = False
        self.tied_embed_init_std = h.tied_embed_init_std
        self.logit_softcap = h.logit_softcap
        self.z_loss_weight = h.z_loss_weight
    ...
    def forward(self, input_ids, target_ids, cu_seqlens=None, max_seqlen=0):
        ...
        loss = F.cross_entropy(logits_flat, flat_targets, reduction="mean")
        lse = torch.logsumexp(logits_flat, dim=-1)
        return loss + self.z_loss_weight * (lse**2).mean()
        ...
        for skip_idx, i in enumerate(dec_iter):
            ...
            if i >= psl and psl > 0:
                ...
                lane0, lane1 = self._parallel_block_with_lora(
                    i, lane0, lane1, x0, lora, slot, q_w, k_w, v_w, out_w, up_w, down_w
                )
            else:
                if skip_idx < self.num_skip_weights and skips:
                    scaled_skip = (
...
def _deserialize_pergroup(blob, num_layers, tmpdir):
    ...
    remainder_state = torch.load(
        io.BytesIO(brotli.decompress(streams[n_groups])),
        map_location="cpu",
        weights_only=False,
    )
    ...
    for group_key in _GROUP_ORDER:
        ...
        for tname, orig_shape in zip(tensor_names, grp_shapes):
            ...
            if len(orig_shape) == 2:
                ...
                if group_key in _SIMSORT_KEYS:
                    perm = np.frombuffer(
                        all_perms[perm_off : perm_off + rows * 2], dtype=np.uint16
                    )
                    perm_off += rows * 2
...
def eval_val(h, device, val_data, model, forward_logits_fn=None):
    ...
    num_windows = max(1, (total_tokens - seq_len + stride) // stride)

    local_batch_windows = max(
        1, h.val_batch_tokens // (h.world_size * h.grad_accum_steps * seq_len)
    )

    win_start_idx = num_windows * h.rank // h.world_size
    win_end_idx = num_windows * (h.rank + 1) // h.world_size
    ...
    with torch.no_grad():
        for batch_start in range(win_start_idx, win_end_idx, local_batch_windows):
            batch_end = min(batch_start + local_batch_windows, win_end_idx)
            actual_batch_size = batch_end - batch_start

            x = torch.empty(
                (actual_batch_size, seq_len), device=device, dtype=torch.int64
            )
            y = torch.empty(
                (actual_batch_size, seq_len), device=device, dtype=torch.int64
            )

            for i, win_idx in enumerate(range(batch_start, batch_end)):
                start_tok = win_idx * stride
                end_tok = start_tok + seq_len + 1
                local = val_data.val_tokens[start_tok:end_tok].to(
                    device=device, dtype=torch.int64, non_blocking=True
                )
                if local.numel() < seq_len + 1:
                    pad_len = seq_len + 1 - local.numel()
                    local = F.pad(local, (0, pad_len), value=0)
                x[i] = local[:-1]
                y[i] = local[1:]

            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                logits = run_forward_logits(x).detach()

            per_token_loss = F.cross_entropy(
                logits.reshape(-1, logits.size(-1)).float(),
                y.reshape(-1),
                reduction="none",
            ).reshape(actual_batch_size, seq_len)

            for i, win_idx in enumerate(range(batch_start, batch_end)):
                start_tok = win_idx * stride
                is_first_window = win_idx == 0
                eval_start = 0 if is_first_window else seq_len - stride
                eval_end = seq_len

                actual_end_tok = min(start_tok + seq_len, total_tokens)
                valid_len = actual_end_tok - start_tok

                if valid_len <= eval_start:
                    continue

                eval_end = min(eval_end, valid_len)

                loss_slice = per_token_loss[i, eval_start:eval_end]
                val_loss_sum += loss_slice.to(torch.float64).sum()
                val_token_count += float(eval_end - eval_start)

                raw_start = start_tok + eval_start
                raw_end = start_tok + eval_end

                if val_data.caseops_enabled and val_data.val_bytes is not None:
                    sidecar_slice = val_data.val_bytes[raw_start + 1 : raw_end + 1].to(
                        device=device, dtype=torch.int32, non_blocking=True
                    )
                    val_byte_count += sidecar_slice.to(torch.float64).sum()
                else:
                    tgt_ids = y[i, eval_start:eval_end]
                    prev_ids = x[i, eval_start:eval_end]
...
def eval_val_ttt_phased(h, base_model, device, val_data, forward_ttt_train):
    ...
    log(
        f"ttt_phased: total_docs:{len(doc_entries)} prefix_docs:{prefix_doc_limit} suffix_docs:{len(doc_entries) - prefix_doc_limit} num_phases:{num_phases} boundaries:{phase_boundaries}"
    )
    ...
    while True:
        ...
        for ci in range(max_nc):
            ...
            if val_data.caseops_enabled and val_data.val_bytes is not None:
                y_idx = (
                    tok_starts.unsqueeze(1) + 1 + col_idx[:context_size].unsqueeze(0)
                )
        ...
        if should_report:
            ...
            elapsed = time.perf_counter() - t_start
            log(
                f"ttp: b{batch_num}/{queue_len} bl:{b_loss:.4f} bb:{b_bpb:.4f} rl:{r_loss:.4f} rb:{r_bpb:.4f} dl:{min(doc_lens)}-{max(doc_lens)} gd:{int(global_ttt_done)}"
            )
            ...
            if should_pause:
                ...
                if h.rank == 0:
                    ...
                    try:
                        ...
                        pass
                    log(
                        f"ttpp: phase:{current_phase + 1}/{num_phases} pd:{prefix_done} gd:{len(scored_docs_for_global)} t:{time.perf_counter() - t_start:.1f}s"
                    )
...
def train_model(h, device, val_data):
    ...
    ema_state = {name: t.detach().float().clone() for (name, t) in _live_state.items()}
...
def _ttt_warmup_compile(h, device):
    """Pre-warm the torch.compile cache for the eval-only TTT graph using same args as eval."""
    log("ttt_compile_warmup: starting (writes to inductor cache)")
    t0 = time.perf_counter()
    ...
    fwd_ttt_compiled = torch.compile(_fwd_ttt_inner, dynamic=True)
    global BOS_ID
    if BOS_ID is None:
        BOS_ID = 1
    bsz = h.ttt_batch_size
    wl = BatchedTTTLoRA(
        bsz,
        ttt_model,
        h.ttt_lora_rank,
    ...
    del wl, wo, ttt_model, fwd_ttt_compiled
    torch.cuda.empty_cache()
    log(f"ttt_compile_warmup: done in {time.perf_counter()-t0:.1f}s")


def do_train(h, device):
    random.seed(h.seed)
    ...
    compiled_eval_forward_logits = torch.compile(
    ...
    timed_eval(
        ...
        val_data,
        compiled_eval_model,
        compiled_eval_forward_logits,
    )
    del eval_model, compiled_eval_model, compiled_eval_forward_logits
    torch._dynamo.reset()
    torch.cuda.empty_cache()
    # Pre-warm TTT compile cache for eval mode
    if h.ttt_enabled:
        _ttt_warmup_compile(h, device)


def do_eval(h, device):
    random.seed(h.seed)
    np.random.seed(h.seed)
    torch.manual_seed(h.seed)
    torch.cuda.manual_seed_all(h.seed)
    ...
    log(
        f"quantized_ttt_phased val_loss:{ttt_val_loss:.8f} val_bpb:{ttt_val_bpb:.8f} eval_time:{1e3*ttt_eval_elapsed:.0f}ms"
    )
    if h.is_main_process:
        print(f"val_loss:{ttt_val_loss:.8f}", flush=True)
        print(f"val_bpb:{ttt_val_bpb:.8f}", flush=True)
        print(f"total_eval_time:{ttt_eval_elapsed:.1f}s", flush=True)
...
def main():
    ...
    if h.is_main_process and h.logfile:
